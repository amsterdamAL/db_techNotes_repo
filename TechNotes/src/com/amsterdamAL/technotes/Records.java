...
public class Records extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//check flag to see which button was clicked to get here, view button? or search button?
		if (Technotes.recordsOrResultsFlag == true){
			
			//if flag was true it was search button
			techhelper.displayResult(Search.uidResult);
			Technotes.recordsOrResultsFlag = false;
		
		//if flag was false it was view button
		}else{
			
			//View button was clicked, displays the last record
			techhelper.getAllRecords();
			//Toast.makeText(getApplicationContext(), "flag was false, so business as usual with getALLRecords", Toast.LENGTH_SHORT).show();;
		}
	}
